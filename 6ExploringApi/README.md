...
## Sepertion of Concerns
for each and everything we have a different projects.(it follows a single responsibility principle), where each or every services has it's own job

###### every/all the services are run on there own specific ports
...
###### Approach 1>
1. (when out webApp loads)-> (we do an API call) -> (Render the data given by API call)
2. suppose out API call takes 500ms to get the data -> so out page will wait for 500ms
3. After 500ms it will render the UI based on given Data

###### Approach 2> (React always use it)
1. Loads up page/WebApp
2. Renders the UI(what the basic data we have, Rendering the Skeleton)
3. API call (after taking data from api call, After that we will rerender our UI or repopulate our UI with new data)
4. Render

###### Approach (2) Gives Better UX -> As React is best in Rendering, so Don't bother about it
--> Hook's is nothing, it's normal JS Function's (ex: useState)
...
useEffect(() => { 
  ...
  const fetchData = async () => {
    const data = await fetch(
      "https://www.swiggy.com/dapi/restaurants/list/v5?lat=22.518&lng=88.3832&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING"
    );
...
- to track teh value of the input box whatever the user is typing in, to get that value
- i have to bind this value wiht a local state variable of React

###### Whenever you change/update a local state variable, React Rereader the component 
###### Whenever state variable update, React trigger a reconciliation cycle(re-renders the component)
###### React is rerendering the whole body component, but it is only updating the input box value inside the DOM.